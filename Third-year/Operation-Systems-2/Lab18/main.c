...
void sort(node_t** head) {
    ...
    do {
        ...
        while (left->next != NULL) {
            ...
            //lock_free (CAS) Сложность реализации. Дорогие атомарные операции, которых к тоже же много
            //mutex хуже тем, что надо продумывать работу с мьютексами для потоков (нет блокировки - нет дедлоков)
            //переключения контекста масштабируемость системы, блокировка потоков
            pthread_mutex_t pm;
